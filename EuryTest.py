...
class Vertex:
    def __init__(self, car_num, busy, speed, leng, source, dest):
...
Cars_Passed = 10  # Number of Cars that have exited system
Time = float(0.0)  # Current Time

K = Vertex(0, 0, 5, 10, "Katipunan", "Katipunan")  # Generation Vertex at LaVista
Gonza = Vertex(0, 0, 5, 10, "Gonza", "Gonza")  # Generation Vertex at Gonzales
TDrive = Vertex(0, 0, 0, 10, "TDrive", "TDrive")  # Generation Vertex at Thornton Drive
K2S = Vertex(0, 0, 5, 100, 0, 0)  # Road between Katip-Gonza_TDrive and Katip-FDRosa-URd, Northbound
K2N = Vertex(0, 0, 5, 100, 0, 0)  # Same as above, but southbound
FDRosa = Vertex(0, 0, 0, 10, 0, 0)  # Generation Vertex at FDRosa
K3 = Vertex(0, 0, 5, 10, 0, 0)  # Generation at Aurora Katipunan
URd = Vertex(0, 0, 5, 10, 0, 0)  # Combination of all four mini roads in the direction of Ateneo
Uturn = Vertex(0, 0, 5, 10, 0, 0)
...
def Distribute_Katipunan(Rate1, Rate2, Rate3, Rate4):
    ...
    while x > 0:
        ...
        x = x - math.floor((Rate1 * K_Gonza.car_num))
        K_K2S.car_num = int((Rate2 * K_Gonza.car_num))
        x = x - math.floor((Rate2 * K_Gonza.car_num))
        K_TDrive.car_num = int((Rate3 * K_Gonza.car_num))
        x = x - math.floor((Rate3 * K_Gonza.car_num))
        K_Uturn.car_num = int((Rate4 * K_Gonza.car_num))
        x = x - math.floor((Rate4 * K_Gonza.car_num))
        if x > 0:
            x = x - 1
...
def Distribute_K2N(Rate1, Rate2):
    ...
    while x > 0:
        ...
        x = x - math.floor((Rate1 * K2N_TDrive.car_num))
        K2N_K.car_num = int((Rate2 * K2N_K.car_num))
        x = x - math.floor((Rate2 * K2N_K.car_num))
        if x > 0:
            x = x - 1
...
def Distribute_K2S(Rate1, Rate2, Rate4):
    ...
    print(K2S.car_num)
    x = math.floor(int(K2S.car_num))
    while x > 0:
        K2S_K3.car_num = math.floor((Rate1 * K2S_K3.car_num))
        x = x - math.floor((Rate1 * K2S_K3.car_num))
        K2S_URd.car_num = math.floor((Rate2 * K2S_URd.car_num))
        x = x - math.floor((Rate2 * K2S_URd.car_num))
        K2S_K2N.car_num = math.floor((Rate4 * K2S_K2N.car_num))
        x = x - math.floor((Rate4 * K2S_K2N.car_num))
        if x > 0:
            x = x - 1
            K2N_K.car_num = K2N_K.car_num + 1
        K2S.car_num = 0
...
def main():
    ...
    while i < 3:
        ...
        Distribute_K2S(0.5, 0.25, 0.25)
        Unassert()
        print("Number of Cars Passed this Cycle:")
        print(Cars_Passed)
        print("TIme Passed This Cycle:")
        print(Time)
        i = i + 1
    flow_rate = float(Cars_Passed / Time)
    print("Total Flow Rate:")
    print(flow_rate)
