...
# Define the BoTorch optimizer
class BoTorchOptimizer:
    def __init__(self, roads, max_cars, max_green_time, min_green_time):
        self.roads = roads
        self.max_cars = max_cars
        self.max_green_time = max_green_time
        self.min_green_time = min_green_time

    def objective_function(self, green_times):
        # Update the simulation with the new green times
        for j in range(len(self.roads)):
            Set_Busy(self.roads[j])
            Time += green_times[j]
            Evaluate_Busy(green_times[j])
            Distribute_Katipunan(0.25, 0.25, 0.25, 0.25)
            Distribute_K2N(0.5, 0.5)
            Distribute_K2S(0.25, 0.25, 0.5)
            Unassert()

        # Calculate the flow rate
        flow_rate = Cars_Passed / Time
        return -flow_rate

    def constraints(self, green_times):
        # Check the green time constraints
        if not all(min_green_time <= green_time <= max_green_time for green_time in green_times):
            return False

        # Check the total green time constraint
        if sum(green_times) > 120:
            return False

        # Check the car number constraints
        for j in range(len(self.roads)):
            if self.roads[j].car_num > self.max_cars:
                return False
    ...
    def optimize(self):
        # Define the BoTorch optimizer
        optimizer = botorch.optim.AcquisitionOptimizer(
            fun=self.objective_function,
            bounds=torch.tensor([[min_green_time, max_green_time]] * len(self.roads)),
        ...
        # Run the BoTorch optimization
        optimal_green_times = optimizer.maximize()
...
def main():
    ...
    botorch_optimizer = BoTorchOptimizer(Roads, 50, 60, 10)

    # Initialize the simulation
    Total_Cars = 0
    for i in range(len(Roads)):
        Roads[i].car_num = 10
        Total_Cars += Roads[i].car_num

    # Run the simulation
    i = 0
    while i < 100:
        # Get the optimal green times
        optimal_green_times = botorch_optimizer.optimize()
