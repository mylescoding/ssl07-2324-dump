...
from botorch.models.exact_marginal_log_likelihood import ExactMarginalLogLikelihood
...
def optimize_green_time(num_iterations, num_samples, bounds):
    ...
    for i in range(num_iterations):
        ...
        candidates, _ = optimize_acqf(
            ...
            bounds=torch.tensor([[bounds[0]], [bounds[1]]]),
            q=num_samples,
            num_restarts=200,
            raw_samples=512,
        ...
        # Update the GP model
        model = SingleTaskGP(torch.cat([model.train_inputs[0], candidates]),
                             torch.cat([model.train_targets, new_results]))
        mll = ExactMarginalLogLikelihood(model.likelihood, model)
