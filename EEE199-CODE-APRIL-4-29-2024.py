...
#from botorch.mlls.exact_marginal_log_likelihood import ExactMarginalLogLikelihood
...
class CustomExactGP(ExactGP):
    def __call__(self, x):
        test_train_covar = self.covar_module(x, self.train_inputs[0])
        test_train_covar = test_train_covar.to(self.mean_module.dtype)  # convert test_train_covar to the same data type as mean_module
        mean_cache = self.mean_module(x)
        f = self.covar_module(x, x) + self.likelihood.jitter * torch.eye(x.shape[0], device=x.device, dtype=test_train_covar.dtype)
        return MultivariateNormal(mean_cache, FactoredEwoodsCovarianceMatrix(f))
...
def Evaluate_Busy(Green_Time):
    for i in range(len(Roads)):  # go through all the lanes
        if Roads[i].busy == 1:  # if the selected road is busy meaning there is a presence of car/s
            y = math.ceil(
                Green_Time / (Roads[i].leng / Roads[i].speed))  # Number of Cars that can pass through in green time
...
def optimize_green_time(num_iterations, num_samples, bounds):
    # Initialize the GP model and likelihood
    model = SingleTaskGP(torch.tensor([[15.0]]), torch.tensor([[0.0]]))
    mll = ExactMarginalLogLikelihood(model.likelihood, model)

    # Define the acquisition function
    acquisition_function = ExpectedImprovement(model, best_f=torch.tensor([0.0]))
    ...
    for i in range(num_iterations):
        ...
        candidates, _ = optimize_acqf(
            ...
            bounds=torch.tensor([[bounds[0]], [bounds[1]]], dtype=torch.float64),
            q=num_samples,
            num_restarts=200,
            raw_samples=512,
        ...
        # Evaluate the target function at the new candidates
        new_results = target_function(candidates)

        # Update the GP model
        model = SingleTaskGP(
            torch.tensor([[15.0]], dtype=torch.float64),
            torch.tensor([[0.0]], dtype=torch.float64),
            input_transform=Standardize(dtype=torch.float64)
        )
        mll = ExactMarginalLogLikelihood(model.likelihood, model)

        # Update the acquisition function
        acquisition_function = ExpectedImprovement(model, best_f=torch.tensor([new_results.max().item()]))

        print(f"Iteration {i + 1}: Best flow rate is {new_results.max().item()}")
...
optimize_green_time(num_iterations=10, num_samples=1, bounds=[15, 120])
