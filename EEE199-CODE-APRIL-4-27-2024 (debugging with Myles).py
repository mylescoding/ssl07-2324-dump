...
def Distribute_K2N(Rate1, Rate2):
    ...
    x = int(K2N.car_num)
    while x > 0:
        r1 = int((Rate1 * K2N.car_num[-1]))
        K2N_TDrive.car_num += r1
        x -= r1
        K2N.car_num -= r1

        r2 = int((Rate2 * K2N.car_num[-1]))
        K2N_K.car_num += r2
        x -= r2
        K2N.car_num -= r2
...
def Distribute_K2S(Rate1, Rate2, Rate4):
    ...
    x = int(K2S.car_num[-1])
    while x > 0:
        r1 = int((Rate1 * K2S.car_num[-1]))
        K2S_K3.car_num += r1
        x -= r1
        K2S.car_num -= r1

        r2 = int((Rate2 * K2S.car_num[-1]))
        K2S_URd.car_num += r2
        x -= r2
        K2S.car_num -= r2

        r4 = int((Rate4 * K2S.car_num[-1]))
        K2S_K2N.car_num += r4
        x -= r4
        K2S.car_num -= r4
...
def Evaluate_Busy(Green_Time):
    for i in range(len(Roads)): #go through all the lanes
        if Roads[i].busy == 1: #if the selected road is busy meaning there is a presence of car/s
            ...
            if (Roads[i].car_num < 0).any():  # Handles negative car number
                y = y + Roads[i].car_num #
                Roads[i].car_num = 0
...
def target_function(Green_Time):
    Total_Cars = 0
    for i in range(len(Roads)):
        Roads[i].car_num = 10
        print(f"Road {Roads[i].name} has {Roads[i].car_num} cars")
