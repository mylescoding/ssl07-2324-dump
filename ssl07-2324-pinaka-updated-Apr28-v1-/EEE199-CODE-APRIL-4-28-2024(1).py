...
def Evaluate_Busy(Green_Time):
    for i in range(len(Roads)): #go through all the lanes
        if int(Roads[i].busy) == 1: #if the selected road is busy meaning there is a presence of car/s
            y = torch.ceil(Green_Time / (Roads[i].leng / Roads[i].speed))  # Number of Cars that can pass through in green time
            Roads[i].car_num = Roads[i].car_num - y  # Remove from road
            if Roads[i].car_num.item() < 0:  # Handles negative car number
                y = y + int(Roads[i].car_num) #
                Roads[i].car_num = 0
        ...
        print(f"Road {Roads[i].name} has {int(Roads[i].car_num)} cars")
    global Time
...
def test_function(Green_Time):
    Total_Cars = 0
    for i in range(len(Roads)):
        int(Roads[i].car_num) = 10
        print(f"Road {Roads[i].name} has {int(Roads[i].car_num)} cars")
        Total_Cars = Total_Cars + int(Roads[i].car_num)
...
def target_function(GreenTime):
    ...
    for i in range(len(Roads)):
        if Roads[i].busy == 1:
            Total = int(Roads[i].car_num) + Total
    Evaluate_Busy(GreenTime)
    flow_rate = []
    flow_rate.append(float(Cars_Passed / Time))
    print("Total Flow Rate:", str(flow_rate))
    return torch.tensor(flow_rate)[0]



def generate_initial_data(n):
    train_greentime = torch.rand(n, 1)
    exact_obj = target_function(train_greentime)
    best_flowrate = exact_obj.max().item()
    return train_greentime, exact_obj.unsqueeze(-1), best_flowrate
...
for_helper_dictionary = { "K2S_Urd":5, "FDRosa_K3":3 , "K2S_K2N":6}

def helper(Diksyonaryo, phase):
    K2S_URd.car_num = Diksyonaryo["K2S_Urd"]
    K2S_K2N.car_num = Diksyonaryo["K2S_K2N"]
    FDRosa_K3.car_num = Diksyonaryo["FDRosa_K3"]
    #Do for every road
    ...
    #Do for all phases
    return optimization_loop(10)

if __name__ == "__main__":
    helper(for_helper_dictionary, 1)